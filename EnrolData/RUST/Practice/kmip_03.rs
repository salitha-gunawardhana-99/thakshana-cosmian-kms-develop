...
async fn handle_create_key_pair(
    ...
    // Update the "device_key_id" field in JSON data
    json_data["KMS_PRIVATE_KEY_ID"] = serde_json::Value::String(private_uid.clone());
    json_data["KMS_PUBLIC_KEY_ID"] = serde_json::Value::String(public_uid.clone());
...
async fn handle_certify(
    ...
    // Update the "device_key_id" field in JSON data
    json_data["KMS_CERT_ID"] = serde_json::Value::String(unique_id.clone());
...
async fn handle_enroldata(
    ...
    let new_create_appended = handle_create(kms, &user, database_params).await?;
    ...
    // Error due to restarting the server?

    // Create Key Pair

    let new_create_key_pair_appended = handle_create_key_pair(kms, &user, database_params).await?;

    let _unused = append_to_response_json(
        "crate/server/src/routes/enrol_data/GenerateEnrolDataResponse.json",
        new_create_key_pair_appended,
    );

    // Certify

    let new_create_appended = handle_certify(kms, &user, database_params).await?;
