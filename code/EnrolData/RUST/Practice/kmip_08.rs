...
pub async fn update_uid_json(uid_path: &str, key_id_ref: &str, unique_id: &str) -> KResult<()> {
    let json_data = fs::read_to_string(uid_path)?;
    let mut json_data: Value = serde_json::from_str(&json_data)?;
    json_data[key_id_ref] = Value::String(unique_id.to_string());
...
pub async fn import_ca(
    ...
    let import_ca_req =
        fs::read_to_string("crate/server/src/routes/enrol_data/operations/Import_ca.json")?;
    let ttlv_operation = serde_json::from_str::<TTLV>(&import_ca_req)?;

    // Define the path for the UID JSON file
    let ca_path = "crate/server/src/routes/enrol_data/id_library/ca.json";

    // Read and parse the existing JSON file
    let json_data: serde_json::Value = serde_json::from_str(&fs::read_to_string(ca_path)?)?;
    ...
    update_uid_json(ca_path, "CA_PRIVATE_KEY_ID", &unique_id).await?;
...
pub async fn handle_create(
    ...
    let create_req =
        fs::read_to_string("crate/server/src/routes/enrol_data/operations/Create.json")?;
    ...
    // fs::write(uid_path, serde_json::to_string_pretty(&json_data)?)?;
    update_uid_json(uid_path, key_id_ref, &unique_id).await?;
...
pub async fn handle_create_key_pair(
    ...
    let create_key_pair_req =
        fs::read_to_string("crate/server/src/routes/enrol_data/operations/CreateKeyPair.json")?;
    ...
    // fs::write(uid_path, serde_json::to_string_pretty(&json_data)?)?;
    update_uid_json(uid_path, private_key_id_ref, &private_uid).await?;
    update_uid_json(uid_path, public_key_id_ref, &public_uid).await?;
...
pub async fn handle_certify(
    ...
    let certify_path = "crate/server/src/routes/enrol_data/operations/Certify.json";
    ...
    // fs::write(uid_path, serde_json::to_string_pretty(&json_data)?)?;
    update_uid_json(uid_path, cert_id_ref, &unique_id).await?;

    // Print the final Unique Identifier
    println!("CERT_ID: {}", unique_id);
...
pub async fn handle_generate_enroldata(
    ...
    let uid_path: &str = "crate/server/src/routes/enrol_data/id_library/uid.json";
    let kms_path: &str = "crate/server/src/routes/enrol_data/id_library/kms.json";
    let gen_response_path: &str =
        "crate/server/src/routes/enrol_data/responses/GenerateEnrolDataResponse.json";
    ...
    let kms_json: serde_json::Value = serde_json::from_str(&fs::read_to_string(kms_path)?)?;
    let ca_id = import_ca(kms, user, database_params).await?;

    // Check if "KMS_PRIVATE_KEY_ID" exists and is valid
    // if let Some(kms_private_key_id) = kms_json.get("KMS_PRIVATE_KEY_ID") {
    ...
    //                 kms_path,
    //                 &ca_id,
    //                 kms_json["KMS_PUBLIC_KEY_ID"].as_str().unwrap(),
    //                 "KMS_CERT_ID",
    //             ),
    //         )
    //         .await?;
...
pub async fn handle_get_enroldata(
    ...
    // Extract UUID
    let uuid = extract_request_info(body, "UUID").await?;
    println!("UUID: {}", uuid);

    // File paths
    let get_req_path = "crate/server/src/routes/enrol_data/operations/Get.json";
    let get_response_path =
        "crate/server/src/routes/enrol_data/responses/GetEnrolDataResponse.json";
    let uid_path = "crate/server/src/routes/enrol_data/id_library/uid.json";
    let kms_path = "crate/server/src/routes/enrol_data/id_library/kms.json";
    let ca_path = "crate/server/src/routes/enrol_data/id_library/ca.json";

    // Load UID JSON data
    let json_data: serde_json::Value = serde_json::from_str(&fs::read_to_string(uid_path)?)?;
    let kms_data: serde_json::Value = serde_json::from_str(&fs::read_to_string(kms_path)?)?;
    let ca_data: serde_json::Value = serde_json::from_str(&fs::read_to_string(ca_path)?)?;
    ...
    async fn fetch_and_update(
        ...
        .await?;
        let _update = update_two_stage_ttlv(get_response_path, tag, &data).await;
    ...
    let _unused = update_two_stage_ttlv(get_response_path, "TimeStamp", &timestamp).await;
